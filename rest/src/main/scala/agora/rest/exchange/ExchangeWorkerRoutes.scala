...
import agora.api.exchange.{ServerSideExchange, UpdateSubscription, _}
...
trait ExchangeWorkerRoutes extends FailFastCirceSupport { self: ExchangeRoutes =>
  ...
  def updateSubscription = post {
    (path("update")) {
      extractMaterializer { implicit mat =>
        ...
        entity(as[UpdateSubscription]) { updateSubscription =>
          complete {
            val ackFuture: Future[UpdateSubscriptionAck] = exchange.updateSubscriptionDetails(updateSubscription)
            ackFuture.map { r =>
              import io.circe.generic.auto._
  ...
  @ApiOperation(
    ...
    notes =
      "If the subscription designated by the request key depends on other subscriptions, it is those subscriptions whose request count will be incremented",
    httpMethod = "POST"
  )
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "body",
                           value = "the subscription from which to request work",
                           required = true,
                           dataTypeClass = classOf[RequestWork],
                           paramType = "body")
    ))
  @ApiResponses(
    Array(
      new ApiResponse(code = 200,
                      message = "Return an ack which details the previous and current number of pending work items",
                      response = classOf[RequestWorkAck])
    ))
  def takeNext = post {
  ...
  @ApiImplicitParams(
    Array(
      new ApiImplicitParam(name = "body",
                           value = "the subscriptions to cancel",
                           required = true,
                           dataTypeClass = classOf[CancelSubscriptions],
                           paramType = "body")
    ))
  @ApiResponses(
    Array(
      new ApiResponse(code = 200,
                      message = "Return an ack which details the previous and current number of pending work items",
                      response = classOf[CancelSubscriptionsResponse])
    ))
