...
import agora.api.exchange.{RequestWorkAck, WorkSubscription, WorkSubscriptionAck}
...
trait MultipartHandlerSupport extends FailFastCirceSupport { self: WorkerRoutes =>
  ...
  def addMultipartHandler(onReq: WorkContext[Multipart.FormData] => Unit)(implicit subscription: WorkSubscription = defaultSubscription,
