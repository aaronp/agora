...
class KeyedPublisherTest extends BaseFlowSpec {
  ...
  "KeyedPublisher.snapshot" should {
    "report the queue and requested amount from subscriptions" in {
      ...
      Pub.subscribe(s1)
      Pub.snapshot() shouldBe PublisherSnapshot(Map("0" -> SubscriberSnapshot(0, 0, 0, 0, 0, Unbounded)))
      Pub.publish("first")
      Pub.snapshot() shouldBe PublisherSnapshot(Map("0" -> SubscriberSnapshot(0, 1, 0, 0, 1, Unbounded)))

      // the second subscription will ask for 123, then get sent 2, leaving 121 requested and 0 queued
      s2.request(123)
      Pub.subscribe(s2)
      Pub.snapshot() shouldBe PublisherSnapshot(
