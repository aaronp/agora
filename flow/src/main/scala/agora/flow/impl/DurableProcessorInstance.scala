...
class DurableProcessorInstance[T](args: Args[T])(implicit execContext: ExecutionContext) extends DurableProcessor[T] with StrictLogging {
  ...
  private[impl] def cancelSubscriber(subscription: DurableSubscription[T]) = {
    val propagateCancel = SubscribersLock.synchronized {
      removeSubscriber(subscription) && subscribers.isEmpty
    }
    if (propagateCancel) {
  ...
  def subscriberCount() = SubscribersLock.synchronized(subscribers.size)
