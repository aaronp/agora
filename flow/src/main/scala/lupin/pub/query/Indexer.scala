...
/** The idea of an 'Indexer' is something which indexes a property A of some type T.
  ...
  * @tparam T
  */
class Indexer[T: Ordering](override val defaultInput: IndexSelection)(implicit execContext : ExecutionContext) extends QueryPublisher[IndexSelection, (T, Int)] with BaseSubscriber[T] {
  ...
  override def subscribeWith(input: IndexSelection, subscriber: Subscriber[_ >: (T, Int)]): Unit = {
    ...
    Publishers.concat(queryPub) { sub =>
      lastReceived match {
