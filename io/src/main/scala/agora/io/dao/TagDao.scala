...
object TagDao {
  def apply[T: Persist: HasId: FromBytes](dir: Path, maxTagLen: Int = 15, maxTagValueLen: Int = 20) = {
    new instances.FileTagDao[T](dir, maxTagLen, maxTagValueLen)
  }

  def apply[T]() = new instances.InMemoryTagDao[T]()

  def noOp[T]: TagDao[T] = new instances.NoOpTagDao[T]
}
